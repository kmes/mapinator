...
describe('Bloodhound', function () {
  ...
  describe('#add', function () {
    it('should add datums to search index', function () {
      ...
      expect(spy).toHaveBeenCalledWith([{ value: 'big' }, { value: 'bigger' }, { value: 'biggest' }]);
    });
  });
  ...
  describe('prefetch', function () {
    ...
    it('should use url or cacheKey to store data locally', function () {
      ...
      expect(this.bloodhound2.storage.set).toHaveBeenCalledWith('data', fixtures.serialized.simple, ttl);
      expect(this.bloodhound2.storage.set).toHaveBeenCalledWith('protocol', 'http:', ttl);
      expect(this.bloodhound2.storage.set).toHaveBeenCalledWith('thumbprint', '%VERSION%!', ttl);
    });
    ...
    it('should load data from provided url', function () {
      ...
      expect(spy1).toHaveBeenCalledWith([{ value: 'big' }, { value: 'bigger' }, { value: 'biggest' }]);

      expect(spy2).toHaveBeenCalledWith([{ value: 'big' }, { value: 'bigger' }, { value: 'biggest' }]);
    });
  ...
  describe('remote', function () {
    it('should perform query substitution on the provided url', function () {
      ...
      this.bloodhound2 = new Bloodhound({
        ...
        remote: {
          url: '/test?q=%QUERY',
          replace: function replace(str, query) {
      ...
      expect(this.bloodhound1.transport.get).toHaveBeenCalledWith('/test?q=one%20two', { type: 'GET', dataType: 'json' }, jasmine.any(Function));

      expect(this.bloodhound2.transport.get).toHaveBeenCalledWith('/test?q=one two', { type: 'GET', dataType: 'json' }, jasmine.any(Function));
    });
  ...
  describe('local/prefetch/remote integration', function () {
    it('duplicates should be removed if dupDetector is provided', function () {
      ...
      this.bloodhound = new Bloodhound({
        ...
        datumTokenizer: datumTokenizer,
        queryTokenizer: queryTokenizer,
        dupDetector: function dupDetector(d1, d2) {
    ...
    it('remote should backfill local/prefetch', function () {
      ...
      expect(spy1).toHaveBeenCalledWith([{ value: 'big' }, { value: 'bigger' }, { value: 'biggest' }]);
      expect(spy2).toHaveBeenCalledWith([{ value: 'bigger' }, { value: 'biggest' }]);
      ...
      runs(function () {
        expect(spy2).toHaveBeenCalledWith([{ value: 'bigger' }, { value: 'biggest' }, { value: 'dog' }]);
      });
